...
export const doCotizacion = async (cot: IDTOCotizacion) => {
    ...
    try {
        const valorResExiste = await Valor_Residual.findAll()
        ...
        const calculatePayment = (interestRate: number, periods: number, presentValue: number, futureValue: number, paymentFrequency: number): number => {
